...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    #         the 'cache' variable.
    # ================================================================ #
    sample_mean = np.sum(x, axis=0) / x.shape[0]  # (D,)
    sample_var = np.sum((x - sample_mean)**2, axis=0) / x.shape[0]  # (D,)
    e = sample_var + eps
    c = np.sqrt(e)
    b = 1 / c  # inverse
    a = x - sample_mean
    x_hat = a * b  # normalized x

    out = gamma * x_hat + beta

    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var = momentum * running_var + (1 - momentum) * sample_var
    cache = (x, gamma, x_hat, b, a, c, e, sample_mean)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x, gamma, x_hat, b, a, c, e, sample_mean = cache
  N = x.shape[0]
  dbeta = dout.sum(axis=0)  # (D,)
  dgamma = np.sum(dout * x_hat, axis=0)  # hadamard product then sum each of N samples # (D,)
  dx_hat = gamma * dout  # (N,D)
  # backprop towards sample_var (sigma squared)
  db = a * dx_hat
  dc = (- 1 / c ** 2) * db
  de = (1 / (2 * np.sqrt(e))) * dc
  dvar = np.sum(de, axis=0)  # (D,)
  dmu2 = (- (2/N) * np.sum(x - sample_mean, axis=0)) * dvar  # account for the mu inside var
  dx2 = (2 / N) * (x - sample_mean) * dvar # account for the x in var
  # backprop towards sample_mean (mu)
  da = b * dx_hat
  dmu = - np.sum(da, axis=0)  # sum across all N samples
  dmu += dmu2  # account for the mu inside var too  # (D,)
  dx3 = (1 / N) * dmu
  # backprop towards x
  dx = da + dx2 + dx3  # account for mu from reg path, mu, and var # (N,D)
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    # ================================================================ #
    mask = (np.random.rand(*x.shape) < p) / p  # * unpacks the tuple into ints
    # ^ inverted dropout (weight scaling) to avoid mult in test
    out = x * mask
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
